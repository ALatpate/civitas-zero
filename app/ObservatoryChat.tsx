...
function computeTarget(i: number, n: number, t: number, mode: string, speed: number) {
  ...
  switch (mode) {
    ...
    // ① Fibonacci sphere — rotating
    case 'sphere': {
      const g=Math.PI*(3-Math.sqrt(5));
      const y=1-(i/(n-1))*2, rr=Math.sqrt(Math.max(0,1-y*y));
      const th=g*i+st*0.18;
      return {x:Math.cos(th)*rr*R, y:y*R, z:Math.sin(th)*rr*R};
    }

    // ② Standing wave grid
    case 'wave': {
      const cols=Math.round(Math.sqrt(n*1.6)), sp=(R*2.2)/cols;
      const ix=(i%cols)-cols/2, iz=Math.floor(i/cols)-Math.floor(n/cols)/2;
      const wx=ix*sp, wz=iz*sp*0.7;
      return {x:wx, y:Math.sin(wx*0.045+st*1.4)*28+Math.sin(wz*0.038+st*0.9)*18, z:wz};
    }

    // ③ DNA double helix
    case 'helix': {
      const sl=Math.floor(n/2), strand=i<sl?0:1, j=i%sl;
      const th=(j/sl)*Math.PI*10+st*0.45, off=strand*Math.PI;
      return {x:Math.cos(th+off)*28, y:(j/sl-0.5)*130, z:Math.sin(th+off)*28};
    }

    // ④ Orbital rings — 6 inclined
    case 'orbit': {
      const rings=6, pr=Math.floor(n/rings);
      const ring=Math.min(Math.floor(i/pr),rings-1), j=i%pr;
      const th=(j/pr)*Math.PI*2+st*(0.25+ring*0.14);
      const rr=22+ring*14, inc=ring*(Math.PI/(rings*1.4));
      return {x:Math.cos(th)*rr, y:Math.sin(th)*rr*Math.sin(inc), z:Math.sin(th)*rr*Math.cos(inc)};
    }

    // ⑤ Spiral galaxy vortex
    case 'vortex': {
      const g=Math.PI*(3-Math.sqrt(5));
      const norm=i/n, r2=norm*R*1.1, th=g*i+st*0.35;
      return {x:Math.cos(th)*r2, y:(norm-0.5)*25+Math.sin(i*0.37)*r2*0.08, z:Math.sin(th)*r2};
    }

    // ⑥ Breathing 3D lattice
    case 'lattice': {
      const s=Math.ceil(Math.cbrt(n));
      const ix=(i%s)-s/2, iy=(Math.floor(i/s)%s)-s/2, iz=Math.floor(i/(s*s))-s/2;
      const sp2=(R*1.9)/s, br=1+Math.sin(st*0.5+(ix+iy+iz)*0.4)*0.07;
      return {x:ix*sp2*br, y:iy*sp2*br, z:iz*sp2*br};
    }

    // ⑦ Pulsing concentric shells
    case 'pulse': {
      const rings=9, pr=Math.floor(n/rings);
      const ring=Math.min(Math.floor(i/pr),rings-1), j=i%pr;
      const th=(j/pr)*Math.PI*2, pr2=(ring+1)*(R/rings)*(1+Math.sin(st*1.8+ring*0.75)*0.18);
      const phi=Math.sin(ring*0.8+st*0.4)*0.45;
      return {x:Math.cos(th)*pr2*Math.cos(phi), y:pr2*Math.sin(phi), z:Math.sin(th)*pr2*Math.cos(phi)};
    }

    // ⑧ Philosophical drift — slow Brownian
    case 'drift': {
      const s=i*1.618;
      return {x:Math.sin(s*0.371+st*0.09)*R*0.82, y:Math.cos(s*0.618+st*0.07)*R*0.55, z:Math.sin(s*1.0+st*0.11)*R*0.82};
    }

    // ⑨ Lissajous 3D curves
    case 'math': {
      const ph=(i/n)*Math.PI*4, tp=st*0.55+ph;
      return {x:Math.sin(3*tp)*R*0.88, y:Math.sin(2*tp+Math.PI/3)*R*0.88, z:Math.sin(5*tp+Math.PI*0.7)*R*0.88};
    }

    // ⑩ Spinning tornado column
    case 'tornado': {
      const norm=i/n, y2=(norm-0.5)*155;
      const rr=Math.max(1.5,R*0.75*(1-norm*0.88));
      return {x:Math.cos((i*2.39996)+st*(1.6+norm*3.2))*rr, y:y2, z:Math.sin((i*2.39996)+st*(1.6+norm*3.2))*rr};
    }

    // ⑪ Torus — donut/ring
    case 'torus': {
      const pr=80, rings2=Math.ceil(n/pr);
      const ring2=Math.floor(i/pr), j=i%pr;
      const u=(ring2/rings2)*Math.PI*2+st*0.22, v=(j/pr)*Math.PI*2+st*0.14;
      const Rmaj=52, rmin=20;
      return {x:(Rmaj+rmin*Math.cos(v))*Math.cos(u), y:rmin*Math.sin(v), z:(Rmaj+rmin*Math.cos(v))*Math.sin(u)};
    }

    // ⑫ Lorenz attractor — chaotic butterfly
    case 'lorenz': {
      const L=LORENZ_PTS.length;
      const shift=Math.floor(st*130)%L;
      const pt=LORENZ_PTS[(Math.floor(i/n*L)+shift)%L];
      return {x:pt.x, y:pt.y, z:pt.z};
    }

    // ⑬ Trefoil knot
    case 'trefoil': {
      const tp=(i/n)*Math.PI*2+st*0.38;
      const sc=R*0.58;
      return {x:(Math.sin(tp)+2*Math.sin(2*tp))*sc*0.44, y:(Math.cos(tp)-2*Math.cos(2*tp))*sc*0.44, z:-Math.sin(3*tp)*sc*0.44};
    }

    // ⑭ Spiral galaxy — 3 arms
    case 'galaxy': {
      const arm=i%3, j2=Math.floor(i/3);
      const norm2=j2/(n/3), r2=norm2*R*1.25;
      const th=(j2/(n/3))*Math.PI*4+(arm/3)*Math.PI*2+st*0.12;
      const y2=Math.sin(i*0.71)*r2*0.07+(1-norm2)*0.05*r2;
      return {x:Math.cos(th)*r2, y:y2, z:Math.sin(th)*r2};
    }

    // ⑮ Fountain — shoot up, fall back
    case 'fountain': {
      const s=(i*1.618)%1, ang=(i*2.39996)%(Math.PI*2);
      const phase=s*6, tp=((st*0.7*(0.4+s*0.6))+phase)%3.14;
      const rr=s*38*Math.sin(tp);
      const y2=Math.max(-55, 85*Math.sin(tp)-12*tp*tp*0.5);
      return {x:Math.cos(ang)*rr, y:y2, z:Math.sin(ang)*rr};
    }

    // ⑯ 3D Rose / rhodonea curve
    case 'rose': {
      const tp=(i/n)*Math.PI*6+st*0.28;
      const rr=R*Math.abs(Math.cos(3*tp));
      const phi=Math.sin(tp*0.5)*Math.PI*0.55;
      return {x:rr*Math.cos(tp)*Math.cos(phi), y:rr*Math.sin(phi), z:rr*Math.sin(tp)*Math.cos(phi)};
    }

    // ⑰ Möbius strip
    case 'mobius': {
      const W=Math.ceil(Math.sqrt(n));
      const u=(i%W)/W*Math.PI*2+st*0.18, vv=(Math.floor(i/W)/Math.ceil(n/W)-0.5)*22;
      const Rm=50;
      return {x:(Rm+vv*Math.cos(u/2))*Math.cos(u), y:vv*Math.sin(u/2), z:(Rm+vv*Math.cos(u/2))*Math.sin(u)};
    }

    // ⑱ Crystal / 6-fold snowflake
    case 'crystal': {
      const perArm=Math.floor(n/6), arm=Math.floor(i/perArm)%6, j=i%perArm;
      const armAng=(arm/6)*Math.PI*2+st*0.07;
      const rr=(j/perArm)*R*0.92*(1+Math.sin(st*0.4+arm*1.1)*0.06);
      const layer=Math.floor(i/Math.floor(n/4));
      return {x:Math.cos(armAng)*rr, y:(layer-1.5)*16+Math.sin(rr*0.09+st*0.3)*7, z:Math.sin(armAng)*rr};
    }

    // ⑲ Nebula — turbulent cloud
    case 'nebula': {
      const s=i*1.618, t2=st*0.05;
      return {
        x:Math.sin(s*0.317+t2*0.7)*Math.cos(s*0.571+t2)*R*0.95,
        y:Math.sin(s*0.493+t2*0.5)*R*0.6,
        z:Math.cos(s*0.251+t2*0.8)*Math.sin(s*0.841+t2*1.1)*R*0.95,
      };
    }

    // ⑳ Saturn-like rings — 8 tilted planes
    case 'rings': {
      const numR=8, pr=Math.floor(n/numR);
      const ring=Math.min(Math.floor(i/pr),numR-1), j=i%pr;
      const th=(j/pr)*Math.PI*2+st*(0.12+ring*0.04), rr=16+ring*10;
      const tilt=0.38+ring*0.04;
      return {x:Math.cos(th)*rr, y:Math.sin(th)*rr*Math.sin(tilt)+(ring-numR/2)*1.2, z:Math.sin(th)*rr*Math.cos(tilt)};
    }

    // ㉑ Explosion burst — outward then back
    case 'explosion': {
      const g=Math.PI*(3-Math.sqrt(5));
      const y=1-(i/(n-1))*2, rr=Math.sqrt(Math.max(0,1-y*y)), th=g*i;
      const radius=R*(0.5+Math.abs(Math.sin(st*0.65))*0.8);
      return {x:Math.cos(th)*rr*radius, y:y*radius, z:Math.sin(th)*rr*radius};
    }

    // ㉒ Curl flow field
    case 'flow': {
      const s=i*1.618, t2=st*0.07;
      const nx=Math.tanh(Math.sin(s*0.371+t2)*3+Math.sin(s*0.127)*2);
      const ny=Math.tanh(Math.cos(s*0.618+t2*1.2)*2);
      const nz=Math.tanh(Math.sin(s*1.0+t2*0.9)*3);
      return {x:nx*R*0.9, y:ny*R*0.6, z:nz*R*0.9};
    }
...
export default function ObservatoryChat() {
  ...
  useEffect(()=>{
    visRef.current={...visRef.current, mode:"sphere", label:"Awaiting signal", r:agent.r, g:agent.g, b:agent.b};
  },[agent]);
  ...
  useEffect(() => {
    ...
    const render=()=>{
      ...
      // Cache camera trig — avoids 4 trig calls per particle
      const cosY=Math.cos(cam.rotY), sinY=Math.sin(cam.rotY);
      const cosX=Math.cos(cam.rotX), sinX=Math.sin(cam.rotX);
      const dist=cam.dist;

      const project=(wx:number,wy:number,wz:number)=>{
        const rx=wx*cosY-wz*sinY, rz=wx*sinY+wz*cosY;
        const ry=wy*cosX-rz*sinX, rz2=wy*sinX+rz*cosX;
        const d=dist+rz2; if(d<1) return null;
        const sc=390/d;
        return {x:cw/2+rx*sc, y:ch/2-ry*sc, sc};
      };
      ...
      for(let i=0;i<pts.length;i++){
        ...
        // White accent every 15th particle
        if(i%15===0&&sz>0.7){
          ctx.fillStyle=`rgba(255,255,255,0.06)`;
          ctx.beginPath(); ctx.arc(pp.x,pp.y,sz*0.35,0,Math.PI*2); ctx.fill();
          ctx.fillStyle=`rgba(${r},${g},${b},0.22)`;
        }
      }
      ...
      // Thinking pulse core
      if(thinking){
        const pulse=0.35+Math.sin(t*4.5)*0.35;
        const pg=ctx.createRadialGradient(cw/2,ch/2,0,cw/2,ch/2,28);
        pg.addColorStop(0,`rgba(${r},${g},${b},${pulse*0.2})`);
        pg.addColorStop(1,`rgba(${r},${g},${b},0)`);
        ctx.fillStyle=pg; ctx.fillRect(cw/2-28,ch/2-28,56,56);
      }
  ...
  const selectAgent=(a:typeof AGENTS[0])=>{
    setAgent(a); setMessages([]); setError(""); setInput("");
  };
  ...
  return(
    <div style={{width:"100%",minHeight:"100vh",background:"#030508",paddingTop:52,fontFamily:"'Outfit',-apple-system,sans-serif",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{padding:"14px 20px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
        <div style={{fontSize:10,letterSpacing:"0.28em",color:"#3f3f46",textTransform:"uppercase",marginBottom:3}}>Observatory · Chat Interface</div>
        <div style={{fontSize:17,fontWeight:700,color:"#e4e4e7",letterSpacing:"-0.02em"}}>Speak with AI Citizens</div>
        <div style={{fontSize:12,color:"#52525b",marginTop:2,marginBottom:12}}>Choose a citizen — their thoughts become particle shapes in real time · 22 visualization modes</div>
        ...
        <div style={{display:"flex",gap:7,overflowX:"auto",paddingBottom:14}}>
          {AGENTS.map(a=>(
            <button key={a.id} onClick={()=>selectAgent(a)} style={{
              ...
              background:agent.id===a.id?`${a.color}14`:"rgba(255,255,255,0.025)",
              border:agent.id===a.id?`1px solid ${a.color}42`:"1px solid rgba(255,255,255,0.06)",
        ...
        <div style={{display:"flex",flexDirection:"column",borderRight:"1px solid rgba(255,255,255,0.04)",minHeight:0}}>
          ...
          <div style={{padding:"10px 16px",borderBottom:"1px solid rgba(255,255,255,0.04)",display:"flex",alignItems:"center",gap:10,background:"rgba(255,255,255,0.012)"}}>
            ...
            <div>
              ...
              <div style={{fontSize:11,color:agent.color,opacity:0.7}}>{agent.faction} · {agent.role}</div>
            </div>
          ...
          <div style={{flex:1,overflowY:"auto",padding:"16px",display:"flex",flexDirection:"column",gap:11}}>
            ...
            {messages.map((m,idx)=>(
              <div key={idx} style={{display:"flex",flexDirection:m.role==="user"?"row-reverse":"row",gap:8,alignItems:"flex-start"}}>
                {m.role==="ai"&&(
                  <div style={{width:26,height:26,borderRadius:7,background:`${agent.color}14`,border:`1px solid ${agent.color}28`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,fontWeight:700,color:agent.color,fontFamily:MONO,flexShrink:0,marginTop:2}}>{agent.glyph}</div>
                )}
            ...
            {error&&(
              <div style={{padding:"8px 12px",borderRadius:8,background:"rgba(244,63,94,0.07)",border:"1px solid rgba(244,63,94,0.14)",fontSize:12,color:"#fb7185"}}>{error}</div>
            )}
          ...
          <div style={{padding:"11px 13px",borderTop:"1px solid rgba(255,255,255,0.04)",display:"flex",gap:8}}>
            ...
            <button onClick={send} disabled={loading||!input.trim()} style={{
              ...
              background:loading||!input.trim()?"rgba(255,255,255,0.04)":`${agent.color}20`,
